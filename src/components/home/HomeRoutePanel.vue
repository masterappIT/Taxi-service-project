<template>
  <view class="route-panel" :class="{ 'airport-mode': mode === 'airport' }" aria-label="路線查詢">
    <image
      class="panel-surface"
    ...
    <view class="ride-tab cross-border" @tap="$emit('update:mode', 'cross-border')">
      <image :src="mode === 'cross-border' ? '/static/home/route/cross-active.svg' : '/static/home/route/cross-inactive.svg'" mode="scaleToFill" />
      <text>{{ '跨境出行' }}</text>
    </view>
    <view class="ride-tab airport" @tap="$emit('update:mode', 'airport')">
      <image :src="mode === 'airport' ? '/static/home/route/airport-active.svg' : '/static/home/route/airport-inactive.svg'" mode="scaleToFill" />
      <text>{{ '接送機' }}</text>
    </view>
    ...
    <view v-else class="airport-fields">
      <view class="route-field flight-field" @tap="$emit('flight')">
        <image class="flight-icon" src="/static/home/route/flight.svg" mode="scaleToFill" />
        <text>{{ flightNumber || '航班號' }}</text>
      </view>
